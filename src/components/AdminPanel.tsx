  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 space-y-4">
        ...
        <p className="text-slate-500 animate-pulse">Loading admin data...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600">Manage users, activate plans, and control system access</p>
        </div>
        <div className="flex items-center gap-4">
          <button 
            onClick={fetchData}
            className="p-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-500 hover:text-brand-primary hover:bg-slate-100 transition-all shadow-sm"
            title="Refresh Data"
          >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="bg-slate-50 border border-slate-200 rounded-full py-2 pl-10 pr-4 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-primary/50 w-64 md:w-80 shadow-sm"
            />
          </div>
        </div>
      </div>

      <div className="glass-card overflow-hidden border-slate-200 shadow-sm bg-white">
                  ...
                  <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-slate-900 font-medium">{user.full_name || 'Anonymous User'}</div>
                    ...
                    <td className="p-4">
                      <div className="flex flex-col gap-1">
                        <select 
                          className="bg-transparent text-sm text-brand-primary font-bold focus:outline-none cursor-pointer hover:text-brand-secondary transition-colors"
                          value={sub?.plan_id || ''}
                      ...
                      {isEditing ? (
                        <div className="flex items-center gap-2">
                          <input 
                            ...
                            className="w-16 bg-slate-50 border border-slate-200 rounded px-2 py-1 text-xs text-slate-900 focus:outline-none"
                          />
                          <button onClick={() => updateCredits(user.id)} className="text-emerald-500 hover:text-emerald-600">
                            <Save className="w-4 h-4" />
                          </button>
                          <button onClick={() => setEditingCredits(null)} className="text-red-500 hover:text-red-600">
                            <X className="w-4 h-4" />
                          </button>
                        </div>
                      ) : (
                        <div className="flex items-center gap-2 group/credits">
                          ...
                          <button 
                            ...
                            className="opacity-0 group-hover/credits:opacity-100 transition-opacity p-1 hover:bg-slate-100 rounded"
                          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-card p-6 border-slate-200 shadow-sm bg-white">
          <div className="text-sm text-slate-500 mb-1 font-medium">Total Users</div>
          <div className="text-3xl font-bold text-slate-900">{users.length}</div>
        </div>
        <div className="glass-card p-6 border-slate-200 shadow-sm bg-white">
          <div className="text-sm text-slate-500 mb-1 font-medium">Active Subscriptions</div>
          <div className="text-3xl font-bold text-emerald-600">
            {users.filter(u => u.user_subscriptions?.[0]?.is_active).length}
          </div>
        </div>
        <div className="glass-card p-6 border-slate-200 shadow-sm bg-white">
          <div className="text-sm text-slate-500 mb-1 font-medium">Pro Users</div>
